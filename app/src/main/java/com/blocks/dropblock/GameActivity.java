...
public class GameActivity extends Activity implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    final Shape[] shapes = new Shape[15];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        NUM_ROWS = Integer.parseInt(prefs.getString("num_rows_preference", "19")) + 6;
        NUM_COLUMNS = Integer.parseInt(prefs.getString("num_columns_preference", "11")) + 6;
    ...
    private void ShapesInit() {
        ...
        shapes[0] = new Shape(a, Color.rgb(255, 0, 255), BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[1][3] = a[2][3] = a[3][3] = 0;

        // Z
        a[2][1] = a[2][2] = a[3][2] = a[3][3] = 1;
        shapes[1] = new Shape(a, Color.rgb(0,255,255), BoardCell.BEHAVIOR_IS_FALLING);
        a[2][1] = a[2][2] = a[3][2] = a[3][3] = 0;

        // I
        a[1][2] = a[2][2] = a[3][2] = a[4][2] = 1;
        shapes[2] = new Shape(a, Color.rgb(128,0,128), BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[3][2] = a[4][2] = 0;

        // O
        a[2][2] = a[2][3] = a[3][2] = a[3][3] = 1;
        shapes[3] = new Shape(a, Color.rgb(0,200,0), BoardCell.BEHAVIOR_IS_FALLING, false);
        a[2][2] = a[2][3] = a[3][2] = a[3][3] = 0;

        // T
        a[1][2] = a[2][2] = a[2][3] = a[3][2] = 1;
        shapes[4] = new Shape(a, Color.rgb(255, 192, 203), BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[2][3] = a[3][2] = 0;

        // S
        a[1][2] = a[2][2] = a[2][3] = a[3][3] = 1;
        shapes[5] = new Shape(a, Color.rgb(255, 215, 0), BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[2][3] = a[3][3] = 0;

        // J
        a[1][3] = a[2][3] = a[3][2] = a[3][3] = 1;
        shapes[6] = new Shape(a, Color.rgb(173,255,50), BoardCell.BEHAVIOR_IS_FALLING);
        ...
        a[1][2] = a[2][2] = a[3][2]  = 1;
        shapes[8] = new Shape(a, Color.rgb(255,0,0), BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[3][2]  = 0;

        a[1][2] = a[2][2]   = 1;
        shapes[9] = new Shape(a, Color.rgb(0,0,255), BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2]   = 0;

        a[1][1] = a[1][2] = a[1][3] = a[2][1] = a[3][1]  = 1;
        shapes[10] = new Shape(a, Color.DKGRAY, BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[1][3] = a[2][1] = a[3][1]  = 0;

        // hard mode

        // +
        a[1][2] = a[2][1] = a[2][2] = a[2][3] = a[3][2] = 1;
        shapes[11] = new Shape(a, Color.GRAY, BoardCell.BEHAVIOR_IS_FALLING, false);
        a[1][2] = a[2][1] = a[2][2] = a[2][3] = a[3][2] = 0;

        // big cube
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 1;
        shapes[12] = new Shape(a, Color.rgb(117, 101, 57), BoardCell.BEHAVIOR_IS_FALLING, false);
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 0;

        //big H
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 1;
        a[1][2] = a[1][3] = a[4][2] = a[4][3] = 0;
        shapes[13] = new Shape(a, Color.rgb(128, 158, 73), BoardCell.BEHAVIOR_IS_FALLING);
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 0;

        a[1][2] = a[2][2] = a[2][3] = a[3][2] = 1;
        shapes[14] = new Shape(a, Color.BLACK, BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[2][3] = a[3][2] = 0;
    }
    ...
    private boolean CreateShape() {
        ...
        if (difficulty.compareTo("Normal") == 0) {
            currentShape = shapes[random.nextInt(11)];
        } else {
    ...
    private boolean Check() {
        int k = 0;
        boolean found = false;
        for (int i = NUM_ROWS - 4; i >= 3; --i) {//26-4
            boolean ok = true;
            for (int j = 3; j < NUM_COLUMNS - 3; ++j) {//16-3 13にすると列のすべてを消す
                if (gameMatrix[i][j].getState() == 0) {
                    ok = false;
                }
            }
